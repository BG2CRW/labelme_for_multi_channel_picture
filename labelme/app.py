...
class MainWindow(QtWidgets.QMainWindow, WindowMixin):
    ...
    def __init__(
        ...
        openNextFolder = action('&Next Fold', self.openNextFolder,
                        shortcuts['open_next'], 'next', u'Open next (hold Ctl+Shift to copy labels)',enabled=False,)

        openPrevFolder = action('&Prev Fold', self.openPrevFolder,
                        shortcuts['open_prev'], 'prev', u'Open next (hold Ctl+Shift to copy labels)',enabled=False,)
        ...
        self.actions = struct(
            ...
            openNextFolder=openNextFolder,openPrevFolder=openPrevFolder,
    ...
    def loadFile(self, filename=None):
        ...
        label_file = os.path.join(dirname, os.path.split(dirname)[1] + '.json')
        if self.output_dir:
            label_file = osp.join(self.output_dir, label_file)

        if QtCore.QFile.exists(label_file) and LabelFile.isLabelFile(label_file):
            try:
    ...
    def saveFileDialog(self):
        ...
        basename = os.path.join(os.path.split(self.filename)[0],os.path.split(self.filename)[0].split('/')[-1])
        if self.output_dir:
    ...
    def openDirDialog(self, _value=False, dirpath=None):
        ...
        self.folderList = []
        total_path = os.path.dirname(targetDirPath)
        dirs = os.listdir(total_path)
        for i,dir in enumerate(dirs):
            new_dir = os.path.join(total_path,dir)
            self.folderList.append(new_dir)
            if dir in targetDirPath:
                self.folderIndex = i
        self.len_folder = len(self.folderList)
        self.importDirImages(targetDirPath)

    def openNextFolder(self, _value=False, dirpath=None):
        if not self.mayContinue():
            return
        self.folderIndex+=1
        if self.folderIndex==self.len_folder:
            self.folderIndex=self.len_folder-1
        self.importDirImages(self.folderList[self.folderIndex])

    def openPrevFolder(self, _value=False, dirpath=None):
        if not self.mayContinue():
            return
        self.folderIndex-=1
        if self.folderIndex==-1:
            self.folderIndex=0
        self.importDirImages(self.folderList[self.folderIndex])
    ...
    def importDirImages(self, dirpath, pattern=None, load=True):
        ...
        self.actions.openNextFolder.setEnabled(True)
        self.actions.openPrevFolder.setEnabled(True)
